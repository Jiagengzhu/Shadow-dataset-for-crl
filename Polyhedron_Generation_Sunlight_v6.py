...
def Generate(dataset):
    ...
    for data in dataset:
        ...
        bpy.data.materials['Light_Material'].node_tree.nodes['Transparent BSDF'].inputs['Color'].default_value = data[3][1]
        light.data.color.r = data[3][1][0]
        light.data.color.g = data[3][1][1]
        light.data.color.b = data[3][1][2]
        step = 1 #step<=8
        for k in range(int(8/step + 1)):
            light.location = [-4+k*step, -4+k*step, 5]
            points = []
            delete_points = []
            for j in range(len(target.data.vertices)):
                ...
                x=v_global.x - v_global.z / 5 * (-4+k*step)
                y=v_global.y - v_global.z / 5 * (-4+k*step)
            ...
            scene.render.filepath='/Users/jiagengz/Documents/blender_scripts/sun-light-samples/' + type + '_' + data[1][0] + '_Size_' + str(data[2]) + '_LightPosition_' + str(k*step) + '_LightColor_' + data[3][0] + '_ShadowArea_' + str(format(polygon_area(shadow) - polygon_area(delete), '.3f')) + '_FloorColor_' + data[3][0] + '.png'
...
light_colors = [('SkyBlue',(0.5,1,1,1)), ('Plum',(1,0.5,1,1)), ('Khaki',(1,1,0.5,1)), ('Lavender',(0.5,0.5,1,1)), ('LightGreen',(0.5,1,0.5,1)), ('Coral',(1,0.5,0.5,1))]
